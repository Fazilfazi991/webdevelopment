...
export async function saveStudioAddressAction(formData: FormData) {
  ...
  await setup.supabase.from("site_domains").update({ status: "removed", is_primary: false }).eq("site_id", siteId).in("domain_type", ["platform_path", "platform_subdomain"]);
  const address = new URL(publicSiteUrl(parsed.data));
  await setup.supabase.from("site_domains").upsert({ site_id: siteId, organization_id: setup.organization.id, domain: `${address.host}${address.pathname}`, domain_type: process.env.NEXT_PUBLIC_PLATFORM_WILDCARD_ENABLED === "true" ? "platform_subdomain" : "platform_path", status: setup.site.publication_status === "published" ? "active" : "pending", verification_status: "verified", ssl_status: "active", is_primary: true, redirect_to_primary: false, verified_at: new Date().toISOString(), created_by: setup.user.id }, { onConflict: "domain" });
...
export async function addCustomDomainAction(formData: FormData) {
  ...
  try {
    ...
    const { error } = await setup.supabase.from("site_domains").upsert({ site_id: siteId, organization_id: setup.organization.id, domain: parsed.data.domain, domain_type: "custom_domain", status: state.verificationStatus === "verified" ? "active" : "verifying", verification_status: state.verificationStatus, ssl_status: state.sslStatus, verification_method: "dns", verification_details: state.instructions ?? {}, is_primary: false, redirect_to_primary: true, created_by: setup.user.id }, { onConflict: "domain" });
...
export async function refreshCustomDomainAction(formData: FormData) {
  ...
  const setup = await context(siteId); const state = await domainProvider().checkDomain(hostname);
  await setup.supabase.from("site_domains").update({ status: state.verificationStatus === "verified" && state.sslStatus === "active" ? "active" : "verifying", verification_status: state.verificationStatus, ssl_status: state.sslStatus, verified_at: state.verificationStatus === "verified" ? new Date().toISOString() : null }).eq("id", domainId).eq("site_id", siteId);
