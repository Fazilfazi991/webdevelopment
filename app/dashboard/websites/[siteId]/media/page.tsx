...
export default async function WebsiteMediaPage({ params }: { params: { siteId: string } }) {
  ...
  return <div className="mx-auto max-w-6xl"><div className="mb-5"><p className="text-xs font-bold uppercase tracking-widest text-brand-700">Website content</p><h1 className="mt-2 text-2xl font-bold text-ink">Photos & Images</h1><p className="mt-1 text-sm text-muted">Update the images used on {setup.site.name}. Each photo shows where it appears.</p></div><ImagesTab siteId={setup.site.id} organizationId={setup.organization.id} context={context} /></div>;
}
