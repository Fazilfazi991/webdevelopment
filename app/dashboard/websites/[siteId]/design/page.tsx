...
export default async function DesignPage({ params, searchParams }: { params: { siteId: string }; searchParams: { message?: string; error?: string } }) {
  ...
  const { site, selection, supabase } = setup;
  ...
  const editorContext = await loadEditorContext(supabase, site.id, canDesign);
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <section><h2 className="text-xl font-bold text-ink">Customize Style</h2><p className="mt-1 text-sm text-muted">Colour palette, font, button, and corner controls use approved presets only.</p><Card className="mt-3 p-5"><DesignTab siteId={site.id} context={editorContext} /></Card></section>
  </div>;
}
