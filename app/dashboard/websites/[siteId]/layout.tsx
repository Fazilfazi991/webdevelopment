...
export default async function WebsiteLayout({ children, params }: { children: React.ReactNode; params: { siteId: string } }) {
  ...
  return <WebsiteWorkspaceShell site={setup.site} sites={setup.sites} palette={[theme?.primary_color ?? "#176b55", theme?.secondary_color ?? "#0f3d35", theme?.accent_color ?? "#d7b98e"]} permissions={{
